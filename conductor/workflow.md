...
7. **Vertical-Slice Delivery (Preferred):** Prefer small end-to-end increments over horizontal layer-by-layer implementation.
8. **Track Kickoff via Integration Test:** For each new track, start by pairing with the user on an end-to-end integration test that validates the feature.
...
0. **Track Kickoff (New Tracks Only): User-Authored Integration Test**
   - Ask the user to provide (or pair-program) an end-to-end integration test that validates the track’s intended behavior.
...
12. **Vertical Slice Validation + Manual Test Guide (After Each Subtask/Slice):**
    - Validate your work by running (as applicable):
      - **Tests** (backend + frontend)
      - **Linting/format**
      - **Type checking**
    - Provide the user a **concrete, step-by-step manual test guide** tailored to the slice’s outcome. Include (as applicable):
      1. How to run the backend (and frontend if relevant)
      2. A specific `curl` command to execute
      3. What response fields/values to verify
      4. For UI changes: exactly what to click/see
