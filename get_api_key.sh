...
Usage:
  ./get_api_key.sh <first_name> <last_name>

Notes:
  - obtains and sets ANTHROPIC_AUTH_TOKEN and LITELLM_API_KEY in your $BASH_PROFILE as environment variable
...
if [[ -z "$FIRST_NAME" || -z "$LAST_NAME" ]]; then
  echo "Error: first_name and last_name are required."
  usage
  exit 1
fi

if [[ "$API_KEY" != sk-* ]]; then
  echo "Error: LITELLM_ADMIN_KEY env variable must be set and begin with 'sk-'."
  exit 1
fi

if [[ -z "$BASE_URL" ]]; then
  echo "No LITELLM_BASE_URL variable set"
  exit 1
fi
...
if [[ -f "$BASH_PROFILE" ]]; then
  ...
  sed -i.bak '/^[[:space:]]*export[[:space:]]\+ANTHROPIC_AUTH_TOKEN=/d' "$BASH_PROFILE"
...
echo "Exported as ANTHROPIC_AUTH_TOKEN and LITELLM_API_KEY to: $BASH_PROFILE"
echo ""
echo "########### TODO ##########"
echo ">>> 1. add the following two lines to your devcontainer.env:"
echo "ANTHROPIC_AUTH_TOKEN=${key_value}"
echo "LITELLM_API_KEY=${key_value}"
echo ">>> 2. Restart your shell session OR run 'source $BASH_PROFILE' to load the new API key into your environment."
echo ">>> 3. Run:  claude -p \"Say Hello!\" "
echo ""
